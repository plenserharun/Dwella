...
export default function RentPage() {
  ...
  const rentProperties = getPropertiesByPriceType("rent");
  const filtered = rentProperties.filter(p => p.title.toLowerCase().includes(searchQuery.toLowerCase()) || p.location.county.toLowerCase().includes(searchQuery.toLowerCase()));

  return (
    <div className="min-h-screen bg-midnight-400">
      <div className="bg-midnight-500 border-b border-white/10">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          <div className="flex items-center gap-2 text-sm text-gray-400 mb-4"><Home className="w-4 h-4" /> / <span>Rent</span></div>
          <h1 className="text-3xl md:text-4xl font-bold text-white mb-2">Properties for <span className="gradient-text-teal">Rent</span></h1>
          <p className="text-gray-400">Find rental properties across Kenya</p>
        </div>
      </div>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
